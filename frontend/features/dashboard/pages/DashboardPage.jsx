...
function MemberDashboard() {
  ...
  return <main className="member-home">
    ...
    <section className="member-sections" aria-label="Contenido principal">
      {sections.map((section) => <article className={`member-section ${section.className}`} key={section.title}>
        <span className="member-section-number">{section.marker}</span><div><p className="member-section-eyebrow">{section.eyebrow}</p><h2>{section.title}</h2><p>{section.description}</p></div>
        {section.href ? <Link className="member-section-action" href={`${section.href}?gym=${encodeURIComponent(gymSlug)}`}>Ver apartado <span aria-hidden="true">&rarr;</span></Link> : <button className="member-section-action" type="button">Ver apartado <span aria-hidden="true">&rarr;</span></button>}
      </article>)}
    </section>
  </main>;
}
